...
class MLP(nn.Module):
    def __init__(self, input_size, output_size, num_hidden_layers, activation):
...
class DeconvolutionModel:
    def __init__(self, input_size, output_size, num_hidden_layers, activation, device=None):
        self.device = device or (
            "cuda" if torch.cuda.is_available() else "mps" if torch.backends.mps.is_available() else "cpu"
        )
        print(f"Using {self.device} device")
        self.model = MLP(input_size, output_size, num_hidden_layers, activation).to(self.device)
